		...
		return this.each(function(){
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
                    ...
                    // console.info(file.type.indexOf("image"));
            ...
            if (files.length < 5) {
              $('.add_upload').fadeIn(300);
					...
					onSuccess: function(file, response) {
						...
						var imgdataObj = JSON.parse(response);
                        // console.log(imgdataObj);
                        // console.log(typeof imgdataObj);
                        imgData.push(imgdataObj.data);
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
